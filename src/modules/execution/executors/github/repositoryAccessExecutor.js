...
export async function execute(node, context) {
  console.log(`[EXEC-TRACE] [repositoryAccessExecutor] Executing ${node?.type} node ${node?.id}`)
  const log = new ExecutionLog(node?.type ?? 'github.repositoryAccess', node?.id ?? 'unknown')

  try {
    if (!node || typeof node !== 'object') {
      throw new GitHubExecutorError('INVALID_NODE', 'Repo access executor received an invalid node object.')
    }
    if (!node.id) {
      throw new GitHubExecutorError('MISSING_NODE_ID', 'Repo access executor received a node without an id.')
    }
    ...
    const { organization, repositories, username, role = 'push' } = node.data
    ...
    if (!Array.isArray(repositories) || repositories.length === 0) {
      throw new GitHubExecutorError('INVALID_REPOSITORIES', `Node ${node.id}: "repositories" must be a non-empty array of repo names.`)
    }
    ...
    if (role && !VALID_REPO_PERMISSIONS.includes(role)) {
      throw new GitHubExecutorError('INVALID_ROLE', `Node ${node.id}: role must be one of [${VALID_REPO_PERMISSIONS.join(', ')}].`)
    }
    ...
    for (const repo of repositories) {
      ...
      try {
        ...
        log.apiRequest('PUT', url, response.status, durationMs, { repo })
        results.push({ repo, status: 'success', githubStatus: response.status })
      } catch (error) {
        allSucceeded = false
        const durationMs = Date.now() - requestStart
        log.apiFailure('PUT', url, error.code || 'API_ERROR', error.message, error.response?.status ?? null, durationMs)
        results.push({ repo, status: 'failed', message: error.message })
      }
    }

    if (!allSucceeded) {
      throw new GitHubExecutorError('GITHUB_PARTIAL_FAILURE', `Node ${node.id}: repo access failed for one or more repos.`, { results })
    }

    return log.finalize('success', `Granted repository access for ${repositories.length} repos to ${username}.`, {
      nodeId: node.id,
      organization,
      repositories,
      username,
